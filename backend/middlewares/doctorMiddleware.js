...
exports.getDoctorProfile = async (req, res, next) => {
    try{
        const doc = Doctor.findById(req.user._id).select('-password -googleId');
...
exports.updateDocProfile = async (req, res, next) => {
    try{
        ...
        const updatedDoc = await Doctor.findByIdAndUpdate(req.user._id, updated, { new : true }).select('-password -googleId');
