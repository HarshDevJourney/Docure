...
export const useAppointmentStore = create<appointmentStore>((set) => ({
  ...
  joinConsultation: async (appointmentID) => {
    ...
    try {
      ...
      set((state) => ({
        appointments: state.appointments.map((apt) =>
          apt._id === appointmentID ? { ...apt, status: "Progress" as const } : apt,
        ),
        currentAppointment:
          state.currentAppointment?._id === appointmentID
            ? { ...state.currentAppointment, status: "Progress" as const }
  ...
  endConsultation: async (appointmentID, notes) => {
    ...
    try {
      ...
      set((state) => ({
        appointments: state.appointments.map((apt) =>
          apt._id === appointmentID ? { ...apt, status: "Completed" as const } : apt,
        ),
        currentAppointment:
          state.currentAppointment?._id === appointmentID
  ...
  updatePrecription: async (appointmentID, file) => {
    ...
    try {
      ...
      if (pescription) {
        set((state) => ({
          appointments: state.appointments.map((apt) =>
            apt._id === appointmentID ? { ...apt, pescription: pescription } : apt,
          ),
          currentAppointment:
            state.currentAppointment?._id === appointmentID
              ? { ...state.currentAppointment, pescription: pescription }
              : state.currentAppointment,
        }));
      }
  ...
  deletePrescription: async (appointmentID) => {
    ...
    try {
      ...
      set((state) => ({
        appointments: state.appointments.map((apt) =>
          apt._id === appointmentID ? { ...apt, pescription: undefined } : apt,
        ),
        currentAppointment:
          state.currentAppointment?._id === appointmentID
            ? { ...state.currentAppointment, pescription: undefined }
            : state.currentAppointment,
      }));
  ...
  markAsFollowUp: async (appointmentID) => {
    ...
    try {
      ...
      set((state) => ({
        ...
        currentAppointment:
          state.currentAppointment?._id === appointmentID
            ? { ...state.currentAppointment, isFollowUp: true }
            : state.currentAppointment,
      }));
