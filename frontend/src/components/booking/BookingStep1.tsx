                ...
                <div className='flex-1'>
                  ...
                  {consultationSpan && (
                    <div className='flex items-center gap-2 text-xs sm:text-sm mb-3 bg-white/70 backdrop-blur-sm rounded-lg px-3 py-2'>
                      ...
                      <span className='text-gray-700'>
                        Each session:{" "}
                        <span className='font-bold text-blue-700'>{consultationSpan} minutes</span>
                      </span>
                    </div>
                  )}
                  ...
                  {docAvailability && (
                    <div className='space-y-2 mb-3'>
                      <div className='flex items-center gap-2 text-xs sm:text-sm bg-white/70 backdrop-blur-sm rounded-lg px-3 py-2'>
                        ...
                        <span className='text-gray-700'>
                          ...
                          <span className='font-bold text-blue-700'>
                            {new Date(docAvailability.startDate).toLocaleDateString("en-US", {
                              month: "short",
                              day: "numeric",
                            })}{" "}
                            -{" "}
                            {new Date(docAvailability.endDate).toLocaleDateString("en-US", {
                              month: "short",
                              day: "numeric",
                              year: "numeric",
                            })}
                      ...
                      {docAvailability.excludedWeekdays &&
                        docAvailability.excludedWeekdays.length > 0 && (
            ...
            <div className='grid grid-cols-2 gap-2 sm:gap-3'>
              {[
                ...
                <button
                  key={type.id}
                  onClick={() =>
                    setFormData({ ...formData, appointmentType: type.id as "video" | "audio" })
                  }
                ...
                >
                  <div className='flex flex-col items-center gap-2 sm:gap-3 text-center'>
                    <div
                      className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center ${formData.appointmentType === type.id ? "bg-blue-600" : "bg-gray-100"}`}
                    >
                      <type.icon
                        size={18}
                        className={
                          formData.appointmentType === type.id ? "text-white" : "text-gray-400"
                        }
                      />
                    </div>
                    <div>
            ...
            <div className='grid grid-cols-7 gap-1.5 sm:gap-2'>
              {days.map((day, index) => {
                const isSelected =
                  day &&
                  formData.date ===
                    `${currentMonth.getFullYear()}-${(currentMonth.getMonth() + 1).toString().padStart(2, "0")}-${day.toString().padStart(2, "0")}`;
              ...
              <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2'>
                <div>
                  ...
                  <p className='text-xs text-gray-500 mt-0.5 sm:mt-1'>
                    <span className='font-semibold text-blue-600'>{availableSlotsCount}</span> slot
                    {availableSlotsCount !== 1 ? "s" : ""} available
                  </p>
                </div>
                  ...
                  <div className='flex items-start gap-2 sm:gap-3 p-3 sm:p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg sm:rounded-xl border border-blue-200'>
                    ...
                    <div className='flex-1'>
                      <p className='text-xs sm:text-sm text-blue-900 leading-relaxed mb-2'>
                        <span className='font-bold'>Tip:</span> Booked slots are unavailable. Select
                        any available time to continue.
                      </p>
