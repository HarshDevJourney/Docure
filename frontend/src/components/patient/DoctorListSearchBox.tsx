...
import { ArrowUpDown, Search, Stethoscope, X } from "lucide-react";
...
interface DoctorListSearchBoxProps {
...
const DoctorListSearchBox: React.FC<DoctorListSearchBoxProps> = ({
    filters,
    onFilterChange,
    onClearFilters,
    minFees,
    ...
    return (
        <div className='max-w-5xl px-4 mx-auto text-center mt-8 md:mt-14'>
            ...
            <div className='relative bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden'>
                ...
                <div className='relative p-4 sm:p-6 space-y-4'>
                    ...
                    <div className='flex flex-col sm:flex-row sm:h-12 border border-gray-200 rounded-xl overflow-hidden w-full shadow-sm hover:shadow-md transition-shadow duration-200 bg-white'>
                        {/* Speciality */}
                        <div className='w-full sm:w-auto flex justify-center items-center sm:justify-start px-3 py-2 sm:py-0 border-b sm:border-b-0 sm:border-r border-gray-200 bg-gray-50/50 sm:bg-transparent'>
                            <Select
                                value={filters.specialization}
                                onValueChange={(value) =>
                                    onFilterChange("specialization", value === "all" ? "" : value)
                                }
                            >
                                <SelectTrigger className='w-full sm:w-auto border border-gray-200 shadow-sm focus:ring-2 focus:ring-blue-500/20 focus-visible:ring-2 focus-visible:ring-blue-500/20 h-10 font-semibold text-blue-700 flex items-center gap-2 bg-white hover:bg-gray-50 transition-colors rounded-lg'>
                                    <Stethoscope className='text-blue-600 w-4 h-4 shrink-0' />
                                    <SelectValue placeholder='Speciality' />
                                </SelectTrigger>
                                <SelectContent className='bg-white border border-gray-200 shadow-lg rounded-lg'>
                                    <SelectItem value="all" className='hover:bg-blue-50 focus:bg-blue-50 cursor-pointer font-medium text-gray-500'>
                                        All Specialities
                                    </SelectItem>
                    ...
                    <div className='w-full flex flex-col sm:flex-row sm:h-12 border border-gray-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow duration-200 bg-white'>
                        {/* FILTER CARD */}
                        <div className='flex-1 px-4 py-3 sm:py-0 border-b sm:border-b-0 sm:border-r border-gray-200 sm:h-full bg-gray-50/30'>
                                ...
                                <div className='grid grid-cols-[auto_1fr] items-center gap-3 sm:border-r sm:border-gray-200 sm:pr-4'>
                                    ...
                                    >
                                        ...
                                        <SelectContent className='bg-white border border-gray-200 shadow-lg rounded-lg'>
                                            ...
                                            {cities.map((city) => (
                                                <SelectItem key={city} value={city.toLowerCase()} className='hover:bg-blue-50 focus:bg-blue-50 cursor-pointer'>
                                                    {city}
                                                </SelectItem>
                                            ))}
                                        </SelectContent>
                        ...
                        <div className='w-auto sm:w-32 md:w-40 h-12 sm:h-auto'>
                            <Button
                                onClick={onApplyFees}
                                className='w-full h-full bg-green-600 hover:bg-green-700 active:bg-green-800 text-white text-sm font-semibold rounded-none sm:rounded-r-xl transition-all duration-200 shadow-sm hover:shadow-md'
                            >
                                Apply Filters
                            </Button>
                        </div>
                        ...
                        <div className='flex flex-wrap items-center gap-2'>
                            {filters.specialization && (
                                <span className='inline-flex items-center gap-1.5 text-xs font-semibold bg-blue-100 text-blue-700 px-3 py-1.5 rounded-full'>
                                    {specializations.find(s => s.toLowerCase() === filters.specialization) || filters.specialization}
                                    <button onClick={() => onFilterChange("specialization", "")} className='hover:text-blue-900 transition-colors'>
                            ...
                            {filters.city && (
                                <span className='inline-flex items-center gap-1.5 text-xs font-semibold bg-green-100 text-green-700 px-3 py-1.5 rounded-full'>
                                    {cities.find(c => c.toLowerCase() === filters.city) || filters.city}
                                    <button onClick={() => onFilterChange("city", "")} className='hover:text-green-900 transition-colors'>
                                        <X className='w-3.5 h-3.5' />
                                    </button>
                                </span>
                            )}
                            {filters.category && (
                                <span className='inline-flex items-center gap-1.5 text-xs font-semibold bg-purple-100 text-purple-700 px-3 py-1.5 rounded-full'>
                                    {filters.category}
                                    <button onClick={() => onFilterChange("category", "")} className='hover:text-purple-900 transition-colors'>
                                        <X className='w-3.5 h-3.5' />
                                    </button>
                                </span>
                            )}
                            {(minFees || maxFees) && (
                                <span className='inline-flex items-center gap-1.5 text-xs font-semibold bg-amber-100 text-amber-700 px-3 py-1.5 rounded-full'>
                                    ₹{minFees || "0"} - ₹{maxFees || "∞"}
                                    <button onClick={() => { onMinFeesChange(""); onMaxFeesChange(""); onApplyFees(); }} className='hover:text-amber-900 transition-colors'>
                                        <X className='w-3.5 h-3.5' />
                                    </button>
                                </span>
                            )}
                            <button
                                ...
                                className='text-xs font-semibold text-gray-500 hover:text-red-500 transition-colors underline underline-offset-2'
                            >
                                Clear all
                            </button>
                        </div>
                        ...
                        <div className='overflow-x-auto scrollbar-hide pb-2 -mx-1 px-1'>
                            <div className='flex gap-2 min-w-max'>
                                {healthcareCategories.map((category) => {
                                    const isActive = filters.category?.toLowerCase() === category.title.toLowerCase();
                                    return (
                                        <button
                                            key={category.id}
                                            type="button"
                                            onClick={() =>
                                                onFilterChange("category", isActive ? "" : category.title)
                                            }
                                        ...
                                        >
                                            <div
                                                className={`
                                                    ...
                                                    border
                                                    ${isActive
                                                        ? `${category.color} border-white/40 ring-2 ring-white/60 scale-105`
                                                        : `${category.color} border-white/20`
                                                    }
                                                `}
                                            >
                                                {/* Hover Overlay */}
            ...
            {/* SORT ROW */}
            <div className='flex items-center gap-3 h-12 mt-4 border border-gray-200 rounded-xl px-4 w-full shadow-sm hover:shadow-md transition-shadow duration-200 bg-white'>
                ...
                <button
                    ...
                    className='h-9 w-9 flex items-center justify-center border border-gray-200 rounded-md hover:border-gray-300 hover:bg-gray-50 transition-all duration-200'
                >
                    <ArrowUpDown className={`w-4 h-4 text-gray-600 transition-transform duration-300 ${sortOrder === "asc" ? "rotate-180" : ""}`} />
                </button>
            </div>
        </div>
    );
