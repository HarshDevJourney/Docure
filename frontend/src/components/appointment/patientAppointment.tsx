...
const UpcomingCard: React.FC<UpcomingCardProps> = ({
  ...
  const grad =
    avatarGradients[apt.doctorID?.avatarColor ?? "default"] ?? "from-blue-400 to-blue-600";
...
const PatientAppointments: React.FC = () => {
  ...
  const { appointments, fetchAppointment, cancelConsultation } = useAppointmentStore();
  ...
  const handleCancel = async (aptId: string) => {
    try {
      await cancelConsultation?.(aptId);
              ...
              <div className='space-y-3'>
                {filtered.map((apt) => (
                  <PastCard
                    ...
                    apt={apt}
                    onRate={handleRate}
                    onDoctorProfile={(id) => router.replace(`/doctor/${id}`)}
                  />
                ))}
              </div>
            )}
