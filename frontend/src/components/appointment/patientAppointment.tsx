...
const StarIcon     = ({ className }: { className?: string }) => <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor" className={className}><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>;
const XIcon        = ({ className }: { className?: string }) => <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>;
const StethIcon    = ({ className }: { className?: string }) => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className={className}><path d="M4.8 2.3A.3.3 0 1 0 5 2H4a2 2 0 0 0-2 2v5a6 6 0 0 0 6 6 6 6 0 0 0 6-6V4a2 2 0 0 0-2-2h-1a.2.2 0 1 0 .3.3"/><path d="M8 15v1a6 6 0 0 0 6 6v0a6 6 0 0 0 6-6v-4"/><circle cx="20" cy="10" r="2"/></svg>;
const DownloadIcon = ({ className }: { className?: string }) => <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className={className}><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>;
const InfoIcon     = ({ className }: { className?: string }) => <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>;
...
const PrescriptionViewer = ({ apt }: { apt: Appointment }) => {
  ...
  if (!rx) return (
    <div className="mt-3 flex items-center gap-2 rounded-xl border border-blue-100 bg-blue-50/40 px-3.5 py-2.5">
      ...
      <p className="text-[11px] text-blue-400 font-medium">Prescription not yet uploaded by the doctor.</p>
    </div>
  );
  return (
...
/* ─────────────────────────────────────────────────────────
   CANCEL MODAL
───────────────────────────────────────────────────────── */
interface CancelModalProps { onConfirm: () => void; onClose: () => void; }
const CancelModal: React.FC<CancelModalProps> = ({ onConfirm, onClose }) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm px-4" onClick={onClose}>
    <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl w-full max-w-sm p-6" onClick={e => e.stopPropagation()}>
      ...
      <h3 className="text-sm font-black text-slate-800 text-center mb-1">Cancel Appointment?</h3>
      <p className="text-xs text-slate-400 text-center mb-5 leading-relaxed">This action cannot be undone. The doctor will be notified of the cancellation.</p>
      <div className="flex gap-2">
        <button onClick={onClose} className="flex-1 rounded-xl border border-slate-200 bg-slate-50 hover:bg-slate-100 py-2 text-xs font-bold text-slate-600 transition-all">Keep it</button>
        <button onClick={onConfirm} className="flex-1 rounded-xl bg-rose-500 hover:bg-rose-600 py-2 text-xs font-bold text-white shadow-md shadow-rose-100 transition-all">Yes, Cancel</button>
      </div>
    </div>
  </div>
);
...
/* ─────────────────────────────────────────────────────────
   STAR RATING
───────────────────────────────────────────────────────── */
interface StarRatingProps { onRate: (rating: number) => void; existing?: number; }
const StarRating: React.FC<StarRatingProps> = ({ onRate, existing }) => {
  ...
  if (selected > 0) return (
    <div className="flex items-center gap-1 mt-2">
      {[1,2,3,4,5].map(i => (
        <StarIcon key={i} className={`w-4 h-4 ${i <= selected ? "text-amber-400" : "text-slate-200"}`} />
      ))}
  ...
  return (
    <div className="flex items-center gap-1 mt-2">
      {[1,2,3,4,5].map(i => (
        <button key={i} onMouseEnter={() => setHover(i)} onMouseLeave={() => setHover(0)} onClick={() => { setSelected(i); onRate(i); }}
          className="p-0.5 transition-transform hover:scale-110 active:scale-95">
          <StarIcon className={`w-4 h-4 transition-colors ${i <= (hover || selected) ? "text-amber-400" : "text-slate-200"}`} />
        </button>
      ))}
...
const UpcomingCard: React.FC<UpcomingCardProps> = ({ apt, onJoin, onCancel, canJoinCall, onDoctorProfile }) => {
  ...
  return (
    <>
      {showCancel && <CancelModal onConfirm={() => { onCancel(apt._id); setShowCancel(false); }} onClose={() => setShowCancel(false)} />}
            ...
            <div className="flex items-start justify-between gap-3 mb-3">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-blue-500 font-semibold mt-0.5">{apt.doctorID?.specialization ?? "General Physician"}</p>
                </div>
              </div>
              <div className="shrink-0">
                {paid
                  ? <span className="inline-flex items-center gap-1 rounded-full border border-emerald-200 bg-emerald-50 px-2 py-1 text-[10px] font-bold text-emerald-700"><CheckIcon className="text-emerald-500" /> ₹{apt.paymentDetails?.totalFees} Paid</span>
                  : <span className="inline-flex items-center gap-1 rounded-full border border-amber-200 bg-amber-50 px-2 py-1 text-[10px] font-bold text-amber-700">₹{apt.paymentDetails?.totalFees} <span className="opacity-70">pending</span></span>
                }
              </div>
            </div>
            ...
            <div className="flex items-center gap-1.5 flex-wrap mb-3">
              ...
              {!isToday && (
                <span className="inline-flex items-center gap-1 rounded-full border border-blue-100 bg-blue-50/40 px-2.5 py-1 text-[10px] font-bold text-blue-500">
                  <CalIcon className="text-blue-400" />{formatDate(apt.date)}
                </span>
              )}
            </div>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              >
                {isLive
                  ...
                  : <><svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor"><polygon points="5 3 19 12 5 21 5 3"/></svg> Join Consultation</>
                }
              </button>
...
const PastCard: React.FC<PastCardProps> = ({ apt, onRate, onDoctorProfile }) => {
  ...
  const accentBar = { Scheduled: "bg-blue-400", Progress: "bg-emerald-500", Completed: "bg-blue-500", Cancelled: "bg-rose-400" }[apt.status];
  const isVideo   = apt.consultationType === "video";
  const paid      = apt.paymentDetails?.paymentStatus === "Paid";
  const relDate   = formatRelative(apt.date);
  const grad      = avatarGradients[apt.doctorID?.avatarColor] ?? "from-blue-400 to-blue-600";
    ...
    <div className="relative rounded-2xl border border-blue-100 bg-white overflow-hidden shadow-sm hover:border-blue-200 hover:shadow-md hover:shadow-blue-50 transition-all duration-200">
      ...
      <div className="px-5 py-4">
        ...
        <div className="flex items-start gap-3 mb-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-1 text-[11px] text-slate-400">
              ...
              {apt.doctorID?.hospital ?? apt.doctorID?.clinic ?? "Docure Health Clinic"}
            </div>
          </div>
          <div className="shrink-0">
            {paid
        ...
        <div className="flex flex-wrap items-center gap-1.5 mb-3">
          <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[11px] font-semibold ${statusMeta.pill}`}>
          ...
          <span className="inline-flex items-center gap-1 rounded-full border border-blue-100 bg-blue-50/60 px-2 py-0.5 text-[11px] font-semibold text-blue-500">
            <ClockIcon className="text-blue-400" />{relDate}
          </span>
        ...
        <p className="text-xs font-semibold text-slate-700">
          ...
          <span className="font-normal text-slate-500">{formatTime(apt.slotStart)} – {formatTime(apt.slotEnd)}</span>
        </p>
        ...
        {apt.status === "Completed" && (
          <StarRating onRate={(rating) => onRate(apt._id, rating)} existing={apt.rating} />
        )}
        ...
        <div className="flex flex-wrap items-center gap-2 mt-3 pt-3 border-t border-blue-50">
          ...
          {apt.status === "Cancelled" && (
            <>
              <span className="text-[10px] font-semibold uppercase tracking-wider text-rose-300">Cancelled</span>
      ...
      <div className="relative flex items-center justify-between flex-wrap gap-3">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-bold uppercase tracking-widest text-blue-200">Your Schedule Today</span>
          </div>
          <p className="text-xl font-black text-white">
            {new Date().toLocaleDateString(undefined, { weekday: "long", month: "long", day: "numeric" })}
          </p>
          <p className="text-xs text-blue-200 mt-0.5">{todayApts.length} appointment{todayApts.length !== 1 ? "s" : ""} today</p>
        </div>
        <div className="flex items-center gap-3">
          {live && (
            <div className="flex items-center gap-2 bg-white/10 backdrop-blur-sm rounded-xl px-3 py-2 border border-white/20">
              ...
              <span className="text-xs font-bold text-white">In session with Dr. {live.doctorID?.name?.split(" ").pop()}</span>
            </div>
          )}
...
const PatientAppointments: React.FC = () => {
  ...
  const pastStats = useMemo(() => {
    ...
    const prescribed = past.filter(a => a.status === "Completed" && a.pescription != null).length;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-white px-4 py-8 sm:px-8">
      ...
      <div className="appt-root relative mx-auto w-full max-w-3xl">
        ...
        {/* HEADER */}
        <div className="mb-7">
          <button onClick={() => router.push("/patient/dashboard")} className="flex items-center gap-1.5 text-xs text-slate-500 hover:text-blue-600 font-semibold mb-5 transition-colors">
          ...
          <div className="flex items-end justify-between gap-4 flex-wrap">
            <div>
              <div className="flex items-center gap-2 mb-1.5">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-600">My Appointments</span>
              </div>
              <h1 className="text-[28px] font-black text-slate-900 leading-tight">My Consultations</h1>
              <p className="mt-1 text-sm text-slate-500 font-medium">Track your visits, prescriptions &amp; follow-ups.</p>
            </div>
            <button
        ...
        {activeTab === "past" && (
          <div className="grid grid-cols-3 gap-3 mb-5">
            {[
              { label: "Completed",    val: pastStats.completed,  cls: "border-blue-200 bg-blue-50 text-blue-700" },
              { label: "Prescriptions",val: pastStats.prescribed, cls: "border-emerald-200 bg-emerald-50 text-emerald-700" },
              { label: "Cancelled",    val: pastStats.cancelled,  cls: "border-rose-200 bg-rose-50 text-rose-700" },
            ].map(s => (
        ...
        <div className="rounded-3xl border border-blue-100 bg-white shadow-xl shadow-blue-100/40 overflow-hidden">
          <div className="flex items-center justify-between px-6 py-3.5 border-b border-blue-50 bg-blue-50/50">
            <span className="text-xs font-bold text-blue-500 flex items-center gap-1.5">
              ...
              {activeTab === "upcoming" ? "Upcoming Appointments" : "Past 7 Days — History"}
            </span>
          ...
          <div className="p-4 sm:p-5">
            {filtered.length === 0 && (
              <div className="flex flex-col items-center justify-center gap-3 py-14 text-center">
                ...
                <p className="text-xs text-slate-400 mb-2">Try adjusting your search or switch tabs.</p>
